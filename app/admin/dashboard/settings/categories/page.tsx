...
export default function CategoriesSettingsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        // اول UI را به‌روزرسانی کن، بعد داده‌ها را ریفرش کن
        setEditingItem(null);
        form.reset();
        ...
        try {
          ...
          console.error('Error refreshing categories after save:', err);
        }
        addNotification({
          type: 'success',
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const res = await fetch(`/api/admin/settings/categories?id=${itemToDelete.id}`, {
        method: 'DELETE',
      });
      const data = await res.json();
      if (data.success) {
