...
export default function DatasetsSettingsPage() {
  ...
  const fetchItems = useCallback(async () => {
    try {
      const res = await fetch('/api/admin/settings/datasets');
  ...
  const handleSubmit = useCallback(
    ...
    [editingItem, fetchItems, addNotification, t]
  );

  const handleDeleteClick = useCallback((item: DatasetItem) => {
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/admin/settings/datasets?id=${itemToDelete.id}`, {
        method: 'DELETE',
      });
      const data = await res.json();
      if (data.success) {
  ...
  }, [itemToDelete, addNotification, t]);
  ...
  return (
    <div className="min-h-screen p-4 md:p-8 relative z-10">
          ...
          >
            <div className="flex-1">
              <AdminSearchInput
                ...
                placeholder={
                  language === 'fa'
                    ? `جستجو بر اساس نام فارسی یا انگلیسی Dataset...`
                    : `Search by Persian or English name for Dataset...`
                }
                isRTL={isRTL}
              />
            </div>
            <AdminImportExport
              type="dataset"
              exportApiPath="/api/admin/settings/datasets/export"
              importApiPath="/api/admin/settings/datasets/import"
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-accent/20">
                      ...
                      <th className="p-3 text-right text-text-secondary">
                        {language === 'fa' ? 'تعداد آیتم‌ها' : 'Items Count'}
                      </th>
                  ...
                  <tbody>
                    {filteredItems.map((item, index) => (
                      ...
                      >
                        ...
                        <td className="p-3">
                          <span className="text-text-secondary">{item.itemsCount}</span>
                        </td>
                        <td className="p-3">
                          <div className="flex gap-2 flex-wrap">
                            <Link href={`/admin/dashboard/settings/datasets/${item.id}`}>
                              <motion.button
                                whileHover={{ scale: 1.05 }}
                                whileTap={{ scale: 0.95 }}
                                className="px-3 py-1 text-sm bg-blue-500/20 hover:bg-blue-500/30 text-blue-300 rounded-lg transition-all duration-200"
                              >
                                {language === 'fa' ? 'مدیریت آیتم‌ها' : 'Manage Items'}
                              </motion.button>
                            </Link>
                            <motion.button
                              whileHover={{ scale: 1.05 }}
            ...
            >
              <div className="p-4 md:p-6 border-b border-accent/20 flex items-center justify-between">
                <h2 className="text-lg md:text-xl font-bold glow-text">
                  {editingItem ? `${t.update} ${t.dataset}` : `${t.create} ${t.dataset}`}
                </h2>
                <button
                  onClick={() => {
                    if (!saving) {
      ...
      <DeleteModal
        ...
        title={`${t.delete} ${t.dataset}`}
        message={
          language === 'fa'
            ? 'آیا مطمئن هستید که می‌خواهید این Dataset را حذف کنید؟ تمام آیتم‌های داخل آن نیز حذف خواهند شد.'
            : 'Are you sure you want to delete this Dataset? All items inside it will also be deleted.'
        }
