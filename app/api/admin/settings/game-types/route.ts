...
export async function GET(request: NextRequest) {
  try {
    ...
    const items = await prisma.gameType.findMany({
    ...
    return NextResponse.json(
      { success: false, error: 'خطا در دریافت انواع بازی' },
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const item = await prisma.gameType.create({
      data,
    });
    ...
    return NextResponse.json(
      { success: false, error: 'خطا در ایجاد نوع بازی' },
      { status: 500 }
    );
  }
}
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const existing = await prisma.gameType.findUnique({ where: { id } });
    if (!existing) {
      return NextResponse.json(
        { success: false, error: 'نوع بازی یافت نشد' },
        { status: 404 }
      );
    }

    const item = await prisma.gameType.update({
      where: { id },
      data: updateData,
    });
    ...
    return NextResponse.json(
      { success: false, error: 'خطا در ویرایش نوع بازی' },
      { status: 500 }
    );
  }
}
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: 'نوع بازی با موفقیت حذف شد',
    });
  } catch (error) {
    console.error('Delete game type error:', error);
    return NextResponse.json(
      { success: false, error: 'خطا در حذف نوع بازی' },
      { status: 500 }
    );
  }
}
