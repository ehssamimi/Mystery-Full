...
// GET: Export لیست انواع بازی به Excel
export async function GET(request: NextRequest) {
  try {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'GameTypes');
    ...
    const filename = `game-types-export-${new Date()
      .toISOString()
      .slice(0, 10)}.xlsx`;
    ...
    console.error('Export game types error:', error);
    return NextResponse.json(
      { success: false, error: 'خطا در Export لیست انواع بازی' },
      { status: 500 }
    );
  }
}
