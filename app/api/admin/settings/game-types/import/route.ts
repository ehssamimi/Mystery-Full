...
// POST: Import انواع بازی از Excel (JSON تبدیل‌شده در کلاینت) با upsert بر اساس id
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('Import game types error:', error);
    return NextResponse.json(
      { success: false, error: 'خطا در Import انواع بازی' },
      { status: 500 }
    );
  }
}
