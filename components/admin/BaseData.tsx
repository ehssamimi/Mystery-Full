...
interface BaseDataProps {
  // API endpoints
  apiPath: string;              // e.g., '/api/admin/settings/categories'
  exportApiPath: string;        // e.g., '/api/admin/settings/categories/export'
  importApiPath: string;        // e.g., '/api/admin/settings/categories/import'
  
  // Resource name (base name) - will be used to generate type, title, and singularName
  baseName: string;             // e.g., 'category' or 'requiredItem'
  
  // Optional: Override auto-generated values
  type?: string;                // Optional override for type (defaults to baseName)
  title?: string;               // Optional override for title (defaults to baseName + 'Management' from translations)
  singularName?: string;        // Optional override for singularName (defaults to baseName from translations)
  
  // Delete confirmation message
  deleteMessage: string;        // Custom delete confirmation message
}
...
export default function BaseData({
  ...
  type: typeOverride,
  title: titleOverride,
  singularName: singularNameOverride,
  deleteMessage,
}: BaseDataProps) {
  ...
  // Auto-generate values from baseName and translations
  const type = typeOverride || baseName;
  
  // Try to get title from translations (e.g., 'categoryManagement' or 'requiredItemManagement')
  const title = titleOverride || (t as any)[`${baseName}Management`] || (t as any)[baseName] || baseName;
  
  // Try to get singular name from translations
  // First try baseName directly (e.g., 'category')
  // Then try baseName + 'Label' (e.g., 'categoryLabel')
  // Fallback to manual mapping for special cases
  let singularName = singularNameOverride;
  if (!singularName) {
    const singularNameMapping: Record<string, { fa: string; en: string }> = {
      ...
      gameType: { fa: 'نوع بازی', en: 'Game Type' },
      datasetItem: { fa: 'آیتم Dataset', en: 'Dataset Item' },
    };
    
    if (singularNameMapping[baseName]) {
      singularName = singularNameMapping[baseName][language];
    } else {
      // Try translations
      singularName = (t as any)[baseName] || (t as any)[`${baseName}Label`] || baseName;
    }
  }

  const [items, setItems] = useState<BaseDataItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [editingItem, setEditingItem] = useState<BaseDataItem | null>(null);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [deleteModalOpen, setDeleteModalOpen] = useState(false);
  const [itemToDelete, setItemToDelete] = useState<BaseDataItem | null>(null);
  ...
  const fetchItems = useCallback(async () => {
    try {
      ...
      const isSuccess = res.ok && (data.success === undefined || data.success === true);

      if (isSuccess) {
        setItems(data.items || []);
      }
    } catch (error) {
      console.error(`Error fetching items from ${apiPath}:`, error);
    } finally {
      setLoading(false);
    }
  }, [apiPath]);
    ...
    try {
      ...
      if (data.success) {
        ...
        try {
          ...
          console.error('Error refreshing items after save:', err);
        }
        addNotification({
          type: 'success',
  ...
  const handleDeleteClick = (item: BaseDataItem) => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const res = await fetch(`${apiPath}?id=${itemToDelete.id}`, {
        method: 'DELETE',
      });
      const data = await res.json();
      if (data.success) {
            ...
            >
              <div className="p-4 md:p-6 border-b border-accent/20 flex items-center justify-between">
                <h2 className="text-lg md:text-xl font-bold glow-text">
                  {editingItem ? `${t.update} ${singularName}` : `${t.create} ${singularName}`}
                </h2>
                <button
                  onClick={() => {
                    if (!saving) {
